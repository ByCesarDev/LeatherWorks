...
foreach ($comMojang in $comMojangPaths) {
    ...
    # Behavior Pack Link
    $bpTarget = Join-Path $comMojang "development_behavior_packs\LeatherWorks BP"
    $bpSource = Join-Path $repo "LeatherWorks BP"
    ...
    # Resource Pack Link
    $rpTarget = Join-Path $comMojang "development_resource_packs\LeatherWorks RP"
    $rpSource = Join-Path $repo "LeatherWorks RP"
